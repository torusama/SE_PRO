...
export default function AppointmentsPage() {
  ...
  return <main className="appointments-page"><header><p>TRAO ĐỔI TRỰC TIẾP</p><h1>Đặt lịch hẹn</h1><span>Chọn thời gian bạn rảnh và nội dung cần trao đổi. Admin sẽ tiếp nhận yêu cầu.</span></header>
    ...
    </section><section className="appointment-panel"><h2>Lịch hẹn của tôi</h2><div className="my-appointments">{!appointments.length ? <p>Chưa có lịch hẹn.</p> : appointments.map(a => <article key={a.id}><div><strong>{a.hostName}</strong><span>{new Date(`${a.appointmentDate}T00:00:00`).toLocaleDateString("vi-VN")} · {a.startTime.slice(0,5)}–{a.endTime.slice(0,5)}</span>{a.note && <small>{a.note}</small>}</div><div><b className={`status ${a.status}`}>{STATUS[a.status] ?? a.status}</b>{["pending","confirmed"].includes(a.status) && <button className="cancel" onClick={() => void cancel(a.id)}>Hủy</button>}</div></article>)}</div></section></div></main>;
}
