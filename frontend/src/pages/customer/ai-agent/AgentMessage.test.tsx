...
describe("AgentMessage actions", () => {
  ...
  it("edits a sent user message and sends the revised content", () => {
    ...
    expect(onEditResend).toHaveBeenCalledWith(message, "Tìm hai lô liền kề");
  });
  ...
  it("sends a backend-marked follow-up when its quoted phrase is clicked", () => {
    const onQuickReply = vi.fn();
    const message: ChatMessage = {
      localId: "assistant-follow-up",
      role: "assistant",
      content: "Mình đã phân tích xong các phương án.",
      createdAt: new Date("2026-01-01T10:00:00"),
      response: {
        ...response,
        quickReplies: [
          {
            id: "analyze-more",
            label: "Tư vấn kỹ các lô",
            message: "Phân tích kỹ hơn các lô vừa đề xuất cho mình.",
          },
          {
            id: "other-plots",
            label: "Gợi ý lô khác",
            message: "Gợi ý cho mình các lô khác theo tiêu chí hiện tại.",
          },
        ],
      },
    };
    ...
    fireEvent.click(screen.getByRole("button", { name: "“Tư vấn kỹ các lô”" }));
    expect(onQuickReply).toHaveBeenCalledWith(
      "Phân tích kỹ hơn các lô vừa đề xuất cho mình.",
    );
  });
});
