...
const knowledgeTypeLabel = (value: string) =>
  ({
    ...
    assistant_instruction: "Hướng dẫn giao tiếp AI",
  ...
  const known = (
    {
      ...
      ai_customer_communication: "Giao tiếp và ứng xử với khách hàng",
...
const knowledgeTypeOptions = [
  ...
  {
    value: "assistant_instruction",
    label: "Hướng dẫn giao tiếp và ứng xử của AI",
  },
  {
...
const knowledgeSourceLabel = (item: KnowledgeProposal) => {
  ...
  if (sourceType === "ai_learning_journal")
    return "Nhật ký AI tự học";
...
const isAutonomouslyReviewedKnowledge = (item: KnowledgeProposal) =>
  item.sourceType?.toLowerCase() === "ai_learning_journal" &&
  item.validationEvidence?.autonomousReview === true;
...
export default function AgentAdminPage() {
  ...
  const saveLearningJournal = async (
    ...
    if (
      !(await confirm({
        ...
        message:
          "Lưu các chỉnh sửa này? Nếu bài học đang được sử dụng, nội dung tương ứng trong Kho tri thức cũng sẽ được cập nhật.",
  ...
  const approveLearningJournal = async (item: LearningJournalItem) => {
    if (
      !(await confirm({
        title: "Duyệt bài học của AI",
        message: `Duyệt “${item.title}” và đưa quy tắc giao tiếp này vào Kho tri thức để AI sử dụng?`,
        confirmLabel: "Duyệt",
      }))
    )
      return;
    setBusy(`learning-journal-approve-${item.learningJournalId}`);
    setError(undefined);
    try {
      await api.patch(
        `/admin/ai-agent/learning-journal/${item.learningJournalId}/approve`,
      );
      await loadData();
    } catch (error) {
      const responseMessage =
      ...
      setError(
        responseMessage ||
          "Không thể duyệt bài học. Nội dung hiện tại chưa được đưa vào Kho tri thức.",
      );
    } finally {
      setBusy(undefined);
    }
          ...
          {tab === "journal" && (
            <div className="agent-admin__section">
              <LearningJournalPanel
                ...
                busy={busy}
                onApprove={approveLearningJournal}
                  ...
                  <tbody>
                    {filteredKnowledge.map((item) => (
                      <tr key={item.knowledgeEntryId}>
                        ...
                        <td>
                          <div className="agent-admin__knowledge-statuses">
                            <span
                              className={`agent-admin__status status-${item.status}`}
                            >
                              {knowledgeStatusLabel(item.status)}
                            </span>
                            {isAutonomouslyReviewedKnowledge(item) && (
              ...
              <div className="agent-admin__knowledge-dialog-body">
                <div className="agent-admin__knowledge-facts">
                  ...
                  <article>
                    ...
                    <strong className="agent-admin__knowledge-dialog-status">
                      {knowledgeStatusLabel(knowledgeDialog.item.status)}
                      {isAutonomouslyReviewedKnowledge(knowledgeDialog.item) && (
                        <span className="agent-admin__status status-ai-reviewed">
                          AI duyệt
                        </span>
                      )}
