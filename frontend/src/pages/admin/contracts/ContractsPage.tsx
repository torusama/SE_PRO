...
export default function ContractsPage() {
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      setBusy("detail");
      void api
        .get(`/admin/contracts/${selectedId}`)
  ...
  const current =
    detail?.id === selectedId
      ? detail
      : contracts.find((item) => item.id === selectedId);
  ...
  async function downloadPdf() {
    ...
    try {
      const content = composeContractDocument(
        current.contractBaseContent ?? current.contractContent ?? "",
        current.inheritanceContent ?? "",
        current.plots ?? [],
      );
  ...
  async function openEvidence(evidence: Evidence) {
    ...
    try {
      const response = await api.get(
        `/admin/contracts/${current.id}/signed-evidence/${encodeURIComponent(evidence.filename)}`,
        { responseType: "blob" },
      );
      const url = URL.createObjectURL(
        new Blob([response.data], { type: evidence.mimeType }),
      );
  ...
  return (
    <div
      className="admin-page contracts-archive"
      style={{ display: "grid", gap: 18 }}
    >
      <header className="admin-page-header">
        <div>
          <h1>Hợp đồng & Sở hữu</h1>
          <p>
            Kho lưu trữ và tra cứu hợp đồng, chứng từ thanh toán, bản ký offline
            và lịch sử sở hữu.
          </p>
        </div>
      </header>
      ...
      >
        ...
        >
          <input
            ...
            placeholder="Tìm mã hợp đồng, khách hàng, mã lô..."
            value={search}
            ...
            {loading ? (
              ...
              filtered.map((item) => (
                ...
                >
                  ...
                  <small>
                    {(item.plotCodes ?? [item.plotCode]).join(", ")} ·{" "}
            ...
            <div style={{ display: "grid", gap: 22 }}>
              <section className="admin-page-header">
                <div>
                  ...
                  <h2 style={{ margin: "5px 0" }}>{current.contractCode}</h2>
                  <p>{statusLabel[current.status] ?? current.status}</p>
                </div>
                <button
              ...
              <section className="admin-detail-grid">
                ...
                <div>
                  <span>CCCD</span>
                  <strong>{current.customerIdCard || "—"}</strong>
                </div>
                <div>
                  <span>Điện thoại</span>
                  <strong>{current.customerPhone || "—"}</strong>
                </div>
                <div>
                  <span>Ngày hợp đồng</span>
                  <strong>{date(current.contractDate)}</strong>
                </div>
                <div>
                  <span>Tổng giá trị</span>
                  <strong>{money.format(current.totalAmount)}</strong>
                </div>
                <div>
                  <span>Đã thanh toán</span>
                  <strong>{money.format(current.paidAmount)}</strong>
                </div>
              </section>
              <section>
                <h3>Các lô trong hợp đồng</h3>
                  ...
                  <table className="admin-table">
                    ...
                    <tbody>
                      {(current.plots ?? []).map((plot) => (
                        <tr key={plot.id}>
                          <td>
                            <strong style={{ color: "#008573" }}>
                              {plot.code}
                            </strong>
                          </td>
                          <td>{plot.zoneName || "—"}</td>
                          <td>{plot.areaSqm ?? "—"} m²</td>
                          <td>{money.format(plot.agreedPrice)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <section>
                <h3>Thông tin thừa kế/thụ hưởng</h3>
                <div
                  style={{
                    whiteSpace: "pre-wrap",
                    padding: 14,
                ...
                >
                  {current.inheritanceContent ||
                    "Không có thông tin thừa kế/thụ hưởng."}
                </div>
              </section>
              <section>
              ...
              <section>
                <h3>Bản hợp đồng đã ký</h3>
                {!current.signedEvidence?.length ? (
                  <p>Chưa có tài liệu ký offline.</p>
                ) : (
                  <div style={{ display: "grid", gap: 8 }}>
                    {current.signedEvidence.map((evidence) => (
                      <button
                        className="admin-secondary-button"
                        style={{ textAlign: "left" }}
                        key={evidence.id}
                        onClick={() => void openEvidence(evidence)}
                        disabled={busy === `evidence-${evidence.id}`}
                      >
                        {evidence.originalName} · {date(evidence.createdAt)}
                      </button>
                    ))}
                  </div>
                )}
              ...
              <section>
                ...
                {!current.ownershipHistory?.length ? (
                  <p>Chưa kích hoạt quyền sở hữu.</p>
                ) : (
                  <div className="admin-table-wrap">
                    <table className="admin-table">
                      <thead>
                      ...
                      <tbody>
                        {current.ownershipHistory.map((row) => (
                          <tr key={row.id}>
                            <td>
                              <strong style={{ color: "#008573" }}>
                                {row.plotCode || "—"}
                            ...
                            <td>{row.isCurrent ? "Hiện hành" : "Lịch sử"}</td>
                            <td>{row.note || "—"}</td>
                          </tr>
                        ))}
                      </tbody>
